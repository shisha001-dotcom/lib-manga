...
(function injectShell() {
  const isStats = location.pathname.split('/').pop() === 'stats.html';

  const navItems = [
    { icon: '📋', label: 'Tất cả',      filter: 'all'        },
    { icon: '🔖', label: 'Chưa đủ bộ', filter: 'incomplete' },
    { icon: '✅', label: 'Full bộ',     filter: 'complete'   },
  ];

  /* ── Các element ngoài .app ── */
  const outsideHtml = `
...
<aside class="sidebar" id="sidebar">
  ...
  /* ── Admin modals + toast (append vào body) ── */
...
<div id="toast"></div>`;
  ...
  /* ── Inject đúng vị trí ── */
  // 1. sidebar-overlay + mobile-header: trước .app
  const appEl = document.querySelector('.app');
  appEl.insertAdjacentHTML('beforebegin', outsideHtml);

  // 2. sidebar: con đầu tiên của .app (trước <main>)
  appEl.insertAdjacentHTML('afterbegin', sidebarHtml);

  // 3. modals + toast: cuối body
